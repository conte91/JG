  ...
  while (keep_on_running)
  {
    ...
    if (f_1_command)
    {
      ...
      case 'l':
        ...
        f_1_command = false;
        f_2_arm_to_move = true; /* goes to f_3_type_move */
        break; /* ihwervb */
      case 'T': /* jgnweriogn  */
      case 't':
        f_1_command = false;
...
/* asdfghjklò xcvbnm ihwervb ieutwrq owieurhg mjuihgv vbnmnbv renvd pwoerithyjgw */
    }
    if (f_2_arm_to_move)
    {
#if (MAX_NUM_ARMS > 1)
    ...
    if (f_4_set_move_parameters)
    {
      ...
      {
        /** HEEEEERE IT IS 
         * Notice that it uses target_pos (inited before) and input_from_user_type (JNT/CART) */
