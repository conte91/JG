...
def on_mouse_click(event, x, y, flags, frame):
    if event == cv2.MOUSE_LBUTTONDOWN:
        selecting = True
    if event == cv2.MOUSE_LBUTTONUP:
        selecting = False
    if selecting and event == cv2.MOUSE_MOVE:
        colors.append(frame[x, y].tolist())

img = cv2.imread("notballs.png")  # "/tmp/myballs.png"
...
for colorName, colorValues in colors.iteritems():
    print "calibrate ", colorName
    while True:
        if colors:
            print colors[-1]
        cv2.imshow('selecting', imgBGR)
        cv2.setMouseCallback('selecting', on_mouse_click, imgHSV)
...
for colorName, colorValues in colors.iteritems():
    ...
    contours, hierarchy = cv2.findContours(match, cv2.RETR_TREE,
